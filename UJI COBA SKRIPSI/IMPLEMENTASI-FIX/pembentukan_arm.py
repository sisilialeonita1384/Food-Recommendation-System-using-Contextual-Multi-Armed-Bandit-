...
random.seed(42)  # supaya hasil random selalu sama
...
def load_data():
    ...
    for kategori, df in data.items():
        if kategori not in ['jus_smoothie', 'nasi']:   #jus_smoothie ga ditampilin lg krn sdh masuk ke kategori buah dan nasi blm dimasukin ke arm.
...
def pilih_slot_susu():
    ...
    terpilih = random.sample(waktu, 2) #contoh: terpilih = ["sarapan", "malam"]
...
def main():
    ...
    if not hasil_validasi["is_valid"]:
        print("\nWARNING: Validasi gagal! Ada arm yang tidak sesuai konsep stok harian.")
    ...
    print("RINGKASAN")
    print("="*60)
    print(f"\n✓ Total arms berhasil dibuat: {len(df_arms):,}")
