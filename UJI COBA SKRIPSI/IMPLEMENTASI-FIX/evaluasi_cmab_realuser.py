...
def get_menu_details(menu_name, sheet_name):
    ...
    try:
        df_menu = DATA_NUTRISI_CACHE[sheet_name] # ambil tabel sesuai kategori
        if sheet_name == 'buah':
            menu = df_menu[df_menu['nama_buah'] == menu_name] # cari baris yg namanya sesuai
            nama_col = 'nama_buah'
        else:
...
def submit():
    ...
    # Simpan Q terbaru dan lanjut ke episode berikutnya 
...
def hasil():
    ...
    # Statistik 
    selected_arm_ids = [h["arm_id"] for h in history] # ambil semua nomor menu (arm_id) dari history
    q_vals_final = [q_values[aid]["q_value"] for aid in selected_arm_ids] # ambil nilai Q dari setiap arm_id tadi
